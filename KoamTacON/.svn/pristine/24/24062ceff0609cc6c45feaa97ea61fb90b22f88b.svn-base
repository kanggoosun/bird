...
<div id="wrapper">
    ...
    <div id="main">
        ...
        <div class="inner">
            ...
            <section>
                <header class="main">
                    <h2><spring:message code='MENU-PURCHASEORDER' text='Purchase Order' /></h2>
                </header>
...
function fn_makeDivBtn(divBtnId) {
    ...
    if (gvGroupCode != null && gvGroupCode != "" && gvGroupCode != "undefined") {
    	...
    	divHtml += "    <li><input type=\"button\" name=\"btnPurchaseView\" id=\"btnPurchaseView\" value=\"<spring:message code='WRD-PURCHASEOD' text='PURCHASE ORDER' />\" class=\"button\" /></li>";
    	divHtml += "</ul>";
...
function fn_makeTblApplicationDataWithPurchase(applicationDataList, paginationHtml, applicationDef) {
    var tblHtml = "";
    var pageHtml = "";
    var buttonHtml = "";
    ...
    tblHtml += "    <thead>";
    tblHtml += "        <tr>";
    tblHtml += "            <th style=\"text-align:center;\"><spring:message code='WRD-PURCHASEORSEQ' text='Purchase Order Seq' /></th>";
    ...
    if (applicationDataList != null && applicationDataList.length > 0) {
        ...
        buttonHtml += "    <li><input type=\"button\" name=\"btnExcelExportPurchase\" id=\"btnExcelExportPurchase\" value=\"<spring:message code='WRD-EXPORT' text='Export' />\" class=\"button special fit\" /></li>";
        //buttonHtml += "    <li><input type=\"button\" name=\"btnExcelImportPurchase\" id=\"btnExcelImportPurchase\" value=\"Import\" class=\"button special fit\" /></li>";
        buttonHtml += "</ul>";
    } else {
        tblHtml += "<tr><td colspan=\"8\" style=\"text-align:center;\"><spring:message code='MSG-VW039' text='There is no data.' /></td></tr>";
    }
    tblHtml += "    </tbody>";
    tblHtml += "    <tfoot style=\"font-size:0.8em;\">";
    tblHtml += "        <tr><td colspan=\"8\" style=\"text-align:center;\" id=\"tdPagination\"></td></tr>";
    tblHtml += "        <tr><td colspan=\"8\" style=\"text-align:right;\" id=\"tdButton\"></td></tr>";
    tblHtml += "    </tfoot>";
    tblHtml += "</table>";
...
var strClickEventNm  = "";
strClickEventNm += "#btnApply, #btnExcelExportPurchase, #btnIntroductionView, #btnPurchaseView";
// mouse click event
...
function fn_eventCallFunction(id) {
    if (id === "btnApply") {
        fn_apply();
    } else if (id === "btnExcelExportPurchase") {
    	fn_excelExport("purchase");
    } else if (id === "btnPurchaseView") {
        fn_makeDivBtn("btnPurchaseView");
        fn_getSearchOptions("MA_PURCHASE");
        fn_getPurchase(1);
    }
...
function fn_getPurchase(pageNo) {
    var param = "";
    param += "curPage="+pageNo;
    param += "&";
    param += "callFunction=fn_getPurchase";
    param += fn_makeSearchParam();
    
    gf_send("<c:url value='/apps/purchase/getPurchase' />", param, "fn_getPurchaseCallback");
}

function fn_getPurchaseCallback(data) {
	var list = data.purchaseList;
    var def = data.purchaseDef;
    var paginationHtml = data.pageInfo.paginationHtml;
    
    fn_makeTblApplicationDataWithPurchase(list, paginationHtml, def);
}
