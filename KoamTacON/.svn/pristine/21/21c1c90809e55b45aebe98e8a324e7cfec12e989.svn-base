...
import com.aisolution.koamtacon.service.service.ApprovalManagementService;
...
@Controller
@RequestMapping("/admin/approvalManagement")
public class ApprovalManagementController {
	private static final Logger log = LoggerFactory.getLogger(ApprovalManagementController.class);
	...
	public String kdcRegistration(@RequestParam Map<String, String> paramMap, Model model) {
		...
		return "/admin/kdcRegApprovalManagement";
	}
	...
	public String kdcApproval(@RequestParam Map<String, String> paramMap, Model model) {
		...
		try {
			approvalManagementService.kdcApproval(paramMap);
	...
	public String getApplicationApprovalList(@RequestParam Map<String, String> paramMap, Model model) {
		...
		paramMap.put("itemNoPerPage", String.valueOf(itemNoPerPage));
		paramMap.put("area", ConfigConstants.AREA);
		Map<String, Object> map = approvalManagementService.getApplicationApprovalList(paramMap);
	...
	public String saveApplicationApproval(@RequestParam Map<String, String> paramMap, Model model) {
		...
		try {
			...
			paramMap.put("area", ConfigConstants.AREA);
			approvalManagementService.saveApplicationApproval(paramMap);
			if (paramMap.get("cbApprovalFlag").equals("2")) {
				if (approvalManagementService.getApplicationGroupCount(paramMap) == 0) {
					approvalManagementService.insertApplicationGroup(paramMap);
				} else {
					paramMap.put("useYn", "Y");
					approvalManagementService.updateApplicationGroup(paramMap);
				}
