...
function fn_getSecurity() {
	...
	if (gfn_checkRequired(frmId) === true) {
		gf_send('<c:out value="${pageContext.request.contextPath}/noLogin/getSecurity" />', "", "fn_getSecurityCallback");
	}
	return false;
}
...
function fn_signin() {
	var frmId = "frmMain";
	gf_sendForm(frmId, '<c:out value="${pageContext.request.contextPath}/noLogin/checkSign" />', "fn_signinCallback");
}
...
$("#closeConfirmPopup").click(function(e){
	...
	gf_submitMovePage('<c:out value="${pageContext.request.contextPath}/noLogin/userProfile" />');
});

function fn_moveToSingup() {
	gf_submitMovePage('<c:out value="${pageContext.request.contextPath}/noLogin/signup" />');
}
...
function fn_resetPassword() {
	...
	if (gfn_checkRequired(frmId) === true) {
		if (gfn_checkFormat(frmId) === true) {
			...
			gf_send('<c:out value="${pageContext.request.contextPath}/noLogin/resetPassword" />', param, "fn_resetPasswordCallback");
...
function fn_userInfoConfirm() {
    ...
    if (gfn_checkRequired(frmId) === true) {
        if (gfn_checkFormat(frmId) === true) {
            ...
            gf_send('<c:out value="${pageContext.request.contextPath}/noLogin/checkUserInfo" />', param, "fn_userInfoConfirmCallback");
            $('#dim-layer-reset').fadeOut();
        }
    }
}
...
function fn_getCheckCountry() {
    ...
    gf_send("<c:url value='/noLogin/checkCountry' />", param, "fn_getCheckCountryCallback");
}
...
function fn_requestDemoAccount() {
	...
	if (gfn_checkRequired(frmId) === true) {
        ...
        if (gfn_checkFormat(frmId) === true) {
    		...
    		gf_sendForm(frmId, "<c:url value='/noLogin/requestDemoAccount' />", "fn_requestDemoAccountCallback");
        }
	}
}
